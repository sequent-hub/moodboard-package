...
**Сохранение (было):** `POST /api/moodboard/save`  
**Сохранение (стало):** `POST /api/v2/moodboard/metadata/save` + `POST /api/v2/moodboard/history/save`
...
**Загрузка (было):** `GET /api/moodboard/{boardId}` или `GET {apiUrl}/load/{boardId}`  
**Загрузка (стало):** `GET /api/v2/moodboard/{moodboard_id}/{version?}`
...
В обработчике сохранения (`v2`: `metadata/save` + `history/save`; legacy: `POST /api/moodboard/save`) залогировать входящие данные одного объекта:
```php
// Laravel пример
...
Дополнительно: сделать `GET /api/v2/moodboard/{moodboard_id}` вручную (через Postman / curl) и убедиться, что в ответе у повёрнутых объектов есть `"transform": { "rotation": ... }`.
